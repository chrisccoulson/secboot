...
// this branch. The function returns the same PCRProtectionProfileBranch
// so that calls may be chained.
//
// Profiles with this command in cannot be used to compute PCR digests
// directly. Instead, the profile will need to be resolved by calling
// ResolveFromTPM. When the profile is resolved, the current value is read
// back from the TPM.
//
...
// for this PCR. The function returns the same PCRProtectionProfile so that
// calss may be chained.
//
// Profiles with this command in cannot be used to compute PCR digests
// directly. Instead, the profile will need to be resolved by calling
// ResolveFromTPM. When the profile is resolved, the current value is read
...
func (c *pcrProtectionProfileComputer) addPCRValueFromTPM(alg tpm2.HashAlgorithmId, pcr int) {
	panic("cannot compute unresolved profile")
}
...
func (p *PCRProtectionProfile) ComputePCRValues() ([]tpm2.PCRValues, error) {
	if p.err != nil {
	...
	if !p.pcrsToReadFromTPM.IsEmpty() {
		return nil, errors.New("cannot compute PCR values because the profile is not resolved from a TPM")
	}
...
func (p *PCRProtectionProfile) ComputePCRDigests(alg tpm2.HashAlgorithmId) (tpm2.PCRSelectionList, tpm2.DigestList, error) {
	// Compute the sets of PCR values for all branches
	values, err := p.ComputePCRValues()
	if err != nil {
		return nil, nil, err
	}
...
type pcrProtectionProfileResolveContext struct {
	values             tpm2.PCRValues
	profile            *PCRProtectionProfile
	currentBranch      *PCRProtectionProfileBranch
	currentBranchPoint *PCRProtectionProfileBranchPoint
}

func (c *pcrProtectionProfileResolveContext) beginBranch(index int) {
	if c.currentBranchPoint == nil {
		c.currentBranch = c.profile.RootBranch()
		return
	}

	c.currentBranch = c.currentBranchPoint.AddBranch()
}

func (c *pcrProtectionProfileResolveContext) addPCRValue(alg tpm2.HashAlgorithmId, pcr int, value tpm2.Digest) {
	c.currentBranch.AddPCRValue(alg, pcr, value)
}

func (c *pcrProtectionProfileResolveContext) addPCRValueFromTPM(alg tpm2.HashAlgorithmId, pcr int) {
	c.currentBranch.AddPCRValue(alg, pcr, c.values[alg][pcr])
}

func (c *pcrProtectionProfileResolveContext) extendPCR(alg tpm2.HashAlgorithmId, pcr int, value tpm2.Digest) {
	c.currentBranch.ExtendPCR(alg, pcr, value)
}

func (c *pcrProtectionProfileResolveContext) beginBranchPoint() {
	c.currentBranchPoint = c.currentBranch.AddBranchPoint()
}

func (c *pcrProtectionProfileResolveContext) endBranchPoint() {
	c.currentBranch = c.currentBranchPoint.EndBranchPoint()
}

func (c *pcrProtectionProfileResolveContext) endBranch() {
	if c.currentBranch == c.profile.RootBranch() {
		return
	}
	c.currentBranchPoint = c.currentBranch.EndBranch()
}

// ResolveFromTPM returns a new profile that contains actual PCR values for any
// PCRs that were added with PCRProtectionProfileBranch.AddPCRValueFromTPM.
// This reads any required PCR values from the supplied TPM context.
func (p *PCRProtectionProfile) ResolveFromTPM(tpm *tpm2.TPMContext) (*PCRProtectionProfile, error) {
	if p.err != nil {
		return nil, fmt.Errorf("cannot resolve because of an error when constructing the profile: %v", p.err)
	}
	if tpm == nil {
		return nil, errors.New("cannot read current PCR values from TPM: no context")
	}

	_, values, err := tpm.PCRRead(p.pcrsToReadFromTPM)
	if err != nil {
		return nil, xerrors.Errorf("cannot read current PCR values from TPM: %w", err)
	}

	resolved := NewPCRProtectionProfile()

	context := &pcrProtectionProfileResolveContext{
		values:  values,
		profile: resolved}
	p.run(context)
	return resolved, nil
