...
func (s *pcrProfileSuite) testPCRProtectionProfile(c *C, data *testPCRProtectionProfileData) {
	...
	pcrs, pcrDigests, err := data.profile.ComputePCRDigests(data.alg)
	c.Assert(err, IsNil)
...
func (s *pcrProfileSuite) TestModifyCompletedBranchPointFails1(c *C) {
	...
	_, _, err := profile.ComputePCRDigests(tpm2.HashAlgorithmSHA256)
...
func (s *pcrProfileSuite) TestAddPCRValueFromTPMFailsWithoutTPM(c *C) {
	...
	_, _, err := profile.ComputePCRDigests(tpm2.HashAlgorithmSHA256)
	c.Check(err, ErrorMatches, `cannot compute PCR values because the profile is not resolved from a TPM`)
}
...
func (s *pcrProfileTPMSuite) TestAddValueFromTPM(c *C) {
	...
	p, err = p.ResolveFromTPM(s.TPM().TPMContext)
	c.Assert(err, IsNil)

	pcrs, digests, err := p.ComputePCRDigests(tpm2.HashAlgorithmSHA256)
	c.Check(err, IsNil)
...
func (s *pcrProfileTPMSuite) TestAddValueFromTPMInvalidPCR(c *C) {
	...
	_, err := p.ResolveFromTPM(s.TPM().TPMContext)
