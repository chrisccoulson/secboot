...
const (
	...
	firmwareProtectedByMeasuredBoot                         // the platform firmware is protected by measured boot (ie, Intel BootGuard in measured boot mode) rather than being fully verified
...
const (
	...
	// PCRProfileOptionsTrustPlatformFirmware can omit PCR0 if a user decides that
	// they trust the platform firmware even though the authenticity of it isn't
	// being guaranteed by a mechanism such as verified boot that is rooted to a
	// hardware root of trust and a piece of immutable code.
	PCRProfileOptionsTrustPlatformFirmware
)
...
func (o *pcrProfileAutoSetPcrsOption) options() ([]PCRProfileEnablePCRsOption, error) {
	...
	default:
		...
		if o.resultFlags&secureBootPolicyOk > 0 {
			...
			isPcr2Supported := func() bool { return o.resultFlags&driversAndAppsOk > 0 }
		...
		if o.resultFlags&firmwareProtectedByMeasuredBoot > 0 && o.flags&PCRProfileOptionsTrustPlatformFirmware == 0 {
			if o.resultFlags&platformFirmwareOk == 0 {
				return nil, errors.New("the platform firmware is partially protected only by measured boot and PCRProfileOptionsTrustPlatformFirmware was not supplied to bypass this, but PCR 0 failed earlier checks")
			}
			opts = append(opts, WithPlatformFirmwareProfile())
		}
		return opts, nil
	}
}
...
func DetectSupport(flags DetectFlags) (pcrAlg tpm2.HashAlgorithmId, options PCRProfileAutoEnablePCRsOption, err error) {
	...
	firmwareProtectionsResult, err := checkPlatformFirmwareProtections()
	switch {
	case err != nil:
		return tpm2.HashAlgorithmNull, errPCRProfileOptions, &PlatformFirmwareProtectionError{err}
	case firmwareProtectionsResult&(firmwareProtectionCPUDebugEnabled|firmwareProtectionCPUDebugAvailable) > 0:
		return tpm2.HashAlgorithmNull, errPCRProfileOptions, &PlatformFirmwareProtectionError{errors.New("CPU debugging features are enabled or available")}
	case firmwareProtectionsResult&firmwareProtectionNoIOMMU > 0:
		return tpm2.HashAlgorithmNull, errPCRProfileOptions, &PlatformFirmwareProtectionError{errors.New("no IOMMU support")}
	case firmwareProtectionsResult&firmwareProtectionMeasuredBootMode > 0:
		resultFlags |= firmwareProtectedByMeasuredBoot
	}
