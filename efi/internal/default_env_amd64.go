...
func (defaultEnvAMD64Impl) ReadMSRs(msr uint32) (map[uint32]uint64, error) {
	...
	case os.IsNotExist(err):
		return nil, errors.New("missing kernel support for reading MSRs")
	case err != nil:
		return nil, err
	}
	defer dir.Close()
	...
	for _, entry := range entries {
		...
		val, err := func(name string) (uint64, error) {
			...
			switch {
			case os.IsNotExist(err):
				return 0, errors.New("missing kernel support for reading MSRs")
			case err != nil:
				return 0, err
			}
			defer f.Close()
			...
			var data [8]byte
			if _, err := f.ReadAt(data[:], int64(msr)); err != nil {
