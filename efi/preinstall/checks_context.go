...
func (c *RunChecksContext) classifyRunChecksError(err error) (kind ErrorKind, args any) {
	...
	if errors.As(err, &tpmhErr) {
		return ErrorKindTPMHierarchyOwned, tpmhErr.Hierarchy
	}
	...
	if errors.As(err, &emptyPcrsErr) {
		return ErrorKindEmptyPCRBanks, emptyPcrsErr.Algs
	}
		...
		for _, err := range pcrAlgErr.BankErrs {
			...
			if !ok {
				...
				if isInvalidTPMResponse(err) {
					...
					return ErrorKindUnexpectedTPMFailure, nil
				}
			...
			return ErrorKindTPMCommandFailed, tpmRsp
		}
	...
	if errors.As(err, &pfPcrErr) {
		return ErrorKindPCRUnusable, internal_efi.PlatformFirmwarePCR
	}

	var pcPcrErr *PlatformConfigPCRError
	if errors.As(err, &pcPcrErr) {
		return ErrorKindPCRUnsupported, internal_efi.PlatformConfigPCR
	}
	...
	if errors.As(err, &daPcrErr) {
		return ErrorKindPCRUnusable, internal_efi.DriversAndAppsPCR
	}

	var dacPcrErr *DriversAndAppsConfigPCRError
	if errors.As(err, &dacPcrErr) {
		return ErrorKindPCRUnsupported, internal_efi.DriversAndAppsConfigPCR
	}
	...
	if errors.As(err, &bmcPcrErr) {
		return ErrorKindPCRUnusable, internal_efi.BootManagerCodePCR
	}

	var bmccPcrErr *BootManagerConfigPCRError
	if errors.As(err, &bmccPcrErr) {
		return ErrorKindPCRUnsupported, internal_efi.BootManagerConfigPCR
	}
	...
	if errors.As(err, &sbPcrErr) {
		return ErrorKindPCRUnusable, internal_efi.SecureBootPolicyPCR
	}
