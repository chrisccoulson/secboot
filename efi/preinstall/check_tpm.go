...
func openAndCheckTPM2Device(env internal_efi.HostEnvironment, flags checkTPM2DeviceFlags) (tpm *tpm2.TPMContext, discreteTPM bool, err error) {
	...
	// and vendor information, and it may round up or down the supplied property to match one
	// of these. In addition to using TPM2_GetCapability to fetch this limited set of properties,
	// only the TPM2_GetTestResult command is usable when the TPM is in failure mode.
	...
	if flags&checkTPM2DevicePostInstall == 0 {
		...
		for _, handle := range []tpm2.Handle{tpm2.HandleLockout, tpm2.HandleOwner, tpm2.HandleEndorsement} {
			...
			if ta.HashAlg != tpm2.HashAlgorithmNull {
				return nil, false, &TPM2HierarchyPolicySetError{Hierarchy: handle}
			}
		}
