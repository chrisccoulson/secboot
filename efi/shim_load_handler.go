...
func (c *shimLoadHandlerConstructor) New(authorities secureBootAuthoritySet, image peImageHandle) (imageLoadHandler, error) {
	...
	// Update the secure boot namespace so that images that are likely
	// rooted to the built-in vendor cert are recognized as belonging to
	// the same namespace.
...
func newShimLoadHandler(authorities secureBootAuthoritySet, image peImageHandle) (imageLoadHandler, error) {
	return newShimLoadHandlerConstructor().New(authorities, image)
}
