...
var makeSecureBootNamespaceRules = func() secureBootNamespaceRules {
	return newSecureBootNamespaceRulesImpl(
		newSecureBootNamespace(
			"Fallback",
			// Shim
			withImageRule(
		...
		newSecureBootNamespace(
			...
			// Pre SBAT shims - unsupported. These will cause an error
			withImageRule(
				imageMatchesAny(
					imageMatchesAll(
						sbatSectionExists,
			...
			withImageRule(
				...
				newGrubLoadHandlerWithFlags(grubChainloaderUsesShimProtocol),
			),
			// Grub
			withImageRule(
				imageMatchesAny(
