...
func (s *shimLoadHandlerSuite) TestNewShimLoadHandler15_7(c *C) {
	...
	var authorities mockSecureBootAuthoritySet

	handler, err := NewShimLoadHandler(&authorities, image.newPeImageHandle())
	c.Assert(err, IsNil)
	...
	c.Check(authorities, DeepEquals, mockSecureBootAuthoritySet{testutil.ParseCertificate(c, canonicalCACert)})
}
...
func (s *shimLoadHandlerSuite) TestNewShimLoadHandler15_6(c *C) {
	...
	handler, err := NewShimLoadHandlerConstructor().WithSbatLevel(sbatLevel).New(&authorities, image.newPeImageHandle())
	c.Assert(err, IsNil)
	c.Assert(handler, testutil.ConvertibleTo, &ShimLoadHandler{})
...
func (s *shimLoadHandlerSuite) TestNewShimLoadHandler15(c *C) {
	...
	_, err := NewShimLoadHandler(&authorities, image.newPeImageHandle())
...
func (s *shimLoadHandlerSuite) TestNewShimLoadHandler15Ubuntu(c *C) {
	...
	handler, err := NewShimLoadHandlerConstructor().WithVersion(MustParseShimVersion("15.2")).New(&authorities, image.newPeImageHandle())
	c.Assert(err, IsNil)
	c.Assert(handler, testutil.ConvertibleTo, &ShimLoadHandler{})
...
func (s *shimLoadHandlerSuite) testMeasureImageLoad(c *C, data *testShimMeasureImageLoadData) {
	...
	s.mockImageLoadHandlers[data.image] = newMockLoadHandler()
	...
	c.Check(err, IsNil)
	c.Check(childHandler, Equals, s.mockImageLoadHandlers[data.image])
	c.Check(ctx.events, DeepEquals, data.expectedEvents)
