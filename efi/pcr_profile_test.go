...
type pcrProfileMockedSuite struct {
	restoreMakeSecureBootNamespaceRules func()
	restoreNewFwLoadHandler             func()
	mockNewFwLoadHandler                func(*tcglog.Log) ImageLoadHandler
	mockImageHandleMixin
	mockImageLoadHandlers
}

func (s *pcrProfileMockedSuite) SetUpSuite(c *C) {
	s.restoreMakeSecureBootNamespaceRules = MockMakeSecureBootNamespaceRules(func() SecureBootNamespaceRules {
		return s
	})
...
func (s *pcrProfileMockedSuite) SetUpTest(c *C) {
	...
	s.mockImageLoadHandlers = make(mockImageLoadHandlers)
}
...
func (s *pcrProfileMockedSuite) TearDownSuite(c *C) {
	...
	if s.restoreMakeSecureBootNamespaceRules != nil {
		s.restoreMakeSecureBootNamespaceRules()
	}
}
...
func (s *pcrProfileMockedSuite) TestPcrProfileGeneratorImageLoadHandlers(c *C) {
	gen := NewPcrProfileGenerator(tpm2.HashAlgorithmSHA256, NewImageLoadSequences())
	c.Check(gen.ImageLoadHandlers(), Equals, s)
}
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileLog(c *C) {
	...
	c.Check(AddPCRProfile(tpm2.HashAlgorithmSHA256, profile.RootBranch(), sequences,
		WithHostEnvironment(efitest.NewMockHostEnvironment(map[efi.VariableDescriptor]*efitest.VarEntry{}, expectedLog)),
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileSimple(c *C) {
	...
	s.mockImageLoadHandlers[images[0]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[0])
	s.mockImageLoadHandlers[images[1]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[1])
	s.mockImageLoadHandlers[images[2]] = newMockLoadHandler()
	...
	c.Check(AddPCRProfile(tpm2.HashAlgorithmSHA256, profile.RootBranch(), sequences,
		WithHostEnvironment(efitest.NewMockHostEnvironment(map[efi.VariableDescriptor]*efitest.VarEntry{
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileSimpleWithLeafBranches(c *C) {
	...
	s.mockImageLoadHandlers[images[0]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[0])
	s.mockImageLoadHandlers[images[1]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[1], digests[2])
	s.mockImageLoadHandlers[images[2]] = newMockLoadHandler()
	s.mockImageLoadHandlers[images[3]] = newMockLoadHandler()
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileSimpleWithBranches(c *C) {
	...
	images := []Image{newMockImage(), newMockImage(), newMockImage(), newMockImage()}
	s.mockImageLoadHandlers[images[0]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[0], digests[1])
	s.mockImageLoadHandlers[images[1]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[2])
	s.mockImageLoadHandlers[images[2]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[2])
	s.mockImageLoadHandlers[images[3]] = newMockLoadHandler()
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileWithParams1(c *C) {
	...
	s.mockImageLoadHandlers[images[0]] = newMockLoadHandler().withExtendPCROnImageLoads(0, digests[0], digests[0])
	s.mockImageLoadHandlers[images[1]] = newMockLoadHandler().
		withExtendPCROnImageLoads(0, digests[1], digests[1]).
		withCheckParamsOnImageStarts(c, params...)
	s.mockImageLoadHandlers[images[2]] = newMockLoadHandler().withCheckParamsOnImageStarts(c, params...)
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileWithRootParams(c *C) {
	...
	s.mockImageLoadHandlers[images[0]] = newMockLoadHandler().
...
func (s *pcrProfileMockedSuite) TestAddPCRProfileWithVariableUpdate(c *C) {
	...
	s.mockImageLoadHandlers[images[2]] = newMockLoadHandler().withCheckVarOnImageStarts(c, "foo", testGuid1, []byte{2}, []byte{2})
