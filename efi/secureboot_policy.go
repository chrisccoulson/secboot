...
func isDbMeasurementEvent(event *tcglog.Event) bool {
	...
	return data.VariableName == efiImageSecurityDatabaseGuid && data.UnicodeName == dbName
}
...
func isSignatureDatabaseMeasurementEvent(event *tcglog.Event) bool {
	...
	data := event.Data.(*tcglog.EFIVariableData)
	switch {
	case data.VariableName == efiGlobalVariableGuid && data.UnicodeName == pkName:
		return true
	case data.VariableName == efiGlobalVariableGuid && data.UnicodeName == kekName:
		return true
	case data.VariableName == efiImageSecurityDatabaseGuid:
		return true
	default:
		return false
	}
